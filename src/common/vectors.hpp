...
template <typename Float> inline Vector3Int FloorVec(glm::vec<3, Float> v)
{

	return Vector3Int(std::floor(v.x), std::floor(v.y), std::floor(v.z));
}

template <typename Float> inline Vector2Int FloorVec(glm::vec<2, Float> v)
{
	return Vector2Int(std::floor(v.x), std::floor(v.y));
}

template <typename Float, int N> inline glm::vec<N, Float> ModVec(glm::vec<N, Float> vec, float mod)
{
