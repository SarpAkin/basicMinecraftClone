...
public:
    uint16_t ID = 0;
    Tile() = default;
    inline Tile(uint16_t ID_)
    {
        ID = ID_;
    }
    Tile(const std::string& Tilename)
    {
        construct(Tilename);
    };
    inline Tile(const char* Tilename)
    {
        std::string name = Tilename;
        construct(name);
    }
    ...
    inline const TileProperties& properties() const
    {
        return TileTypes[ID];
    }
